...
export function DashboardOverview() {
  ...
  const avgTrustScore = Math.round(
    mockArticles.reduce((sum, article) => sum + article.vertrouwensscore, 0) / totalArticles
  );
  const recentArticles = mockArticles.slice(0, 6);
  const totalCitations = mockArticles.reduce((sum, article) => sum + article.citations, 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border-l-4 border-purple-500">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-3xl font-bold text-[#1C3664]">{totalCitations}</span>
          </div>
          <p className="text-gray-600">Totaal Citaties</p>
        </div>
